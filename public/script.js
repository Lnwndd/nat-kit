...
let safetychat = [
    {
        ...
        is_name: true,
        create_date: 0,
        mes: "\n캐릭터 / 채팅이 삭제되어 안전한 곳으로 돌아왔습니다.\n새로운 캐릭터를 선택해주세요!\n\n",
    },
];
let chat_create_date = 0;

let prev_selected_char = null;
const default_ch_mes = "안녕하세요";
...
const system_messages = {
    help: {
        ...
        is_name: true,
        mes: [
            '\n안녕하세요!\n자주 사용되는 채팅 서식 명령어에 대해 알려드릴게요.\n해당 명령어를 통해 캐릭터가 대화와 상황을 명확하게 이해할 수 있어요!',
            '<ol>',
            '<li><tt>&#42;내용&#42;</tt> – 행동, 상황을 표현할 때 사용합니다.</li>',
            '<li><tt>&#42;&#42;내용&#42;&#42;</tt> – 소리치거나 내용을 강조할 때 사용합니다.</li>',
            '<li><tt>(내용)</tt> – 내용을 강조하거나 설명할 때 사용합니다.</li>',
            '<li><tt>OOC:내용</tt> – 캐릭터가 아닌 AI 에게 직접 명령할 때 사용합니다.</li>',
            '<li><tt>{{내용}}</tt> – 내용에 편향 가중치를 설정합니다.</li>',
            '<li><tt>{{}}</tt> – 이전에 설정한 편향 가중치를 취소합니다.</li>',
            '</ol>',
			'\n명령어는 서로 조합이 가능하며, 꼭 서식을 지키지 않아도 사용이 가능합니다.\n더 많은 명령어와 정보는 <a target="_blank" href="https://docs.google.com/document/d/14iadec1YaVqv9W6LyZZodmovR2v9nRxRMCQuFxjdREE/edit">AI 채팅채널 FAQ</a> 에서 확인해주세요!',
        ].join('')
    },
    welcome:
    {
        ...
        is_name: true,
        mes: [
			'실리 넷데브 AI 에 오신 것을 환영합니다!\n현재 실리 와이푸 v 0.17 + 넷데브 컨버젼 + 골든 커스텀이 적용되어 있습니다.\n실리 넷데브는 정식 지원 전까지 임시 (업데이트 X) 로 사용됩니다.\n아래 링크에서 자세한 사항을 확인하세요.',
            '<ul>',
            '<li>실리 넷데브 - 메인 :<a target="_blank" href="https://arca.live/b/characterai/74004375"> Silly Natdev</a></li>',
            '<li>실리 넷데브 - 확장 기능 :<a target="_blank" href="https://arca.live/b/characterai/72754580">  Silly Extension</a></li>',
            '<li>실리 넷데브 - 원본 자료 :<a target="_blank" href="https://arca.live/b/characterai/73854128"> Silly Natdev + OG</a></li>',
            '<li>실리 넷데브 - 백그라운드 :<a target="_blank" href="https://arca.live/b/characterai/73750332"> Silly Natdev + BGI</a></li>',
            '<li>실리 넷데브 - 통합 사용 설명서 :<a target="_blank" href="https://arca.live/b/characterai/73992817"> Silly Natdev + Guide Book</a></li>',
            '</ul>',
            '명령어 도움말을 불러오시려면 <tt>/?</tt> 를 입력해주세요.\n더 많은 정보는 <a target="_blank" href="https://docs.google.com/document/d/14iadec1YaVqv9W6LyZZodmovR2v9nRxRMCQuFxjdREE/edit">AI 채팅채널 FAQ</a> 에서 확인해주세요!',
			'\nㅤ\n**[ WyFO9YRyeV2 : Natdev / 타코웰치 : Waifu / 골든햄스터 : Custom ]**',
        ].join('')
    },
    group: {
        name: systemUserName,
    ...
    empty: {
        ...
        is_system: true,
        is_name: true,
        mes: "이곳엔 아무도 없네요... <b>힌트&#58;</b> 그룹에 새로운 멤버를 추가하세요!",
    },
    generic: {
        name: systemUserName,
        force_avatar: system_avatar,
    ...
    bookmark_created: {
        ...
        is_system: true,
        is_name: true,
        mes: `북마크가 생성되었습니다! 북마크를 열려면 여기를 클릭하세요 : <a class="bookmark_link" file_name="{0}" href="javascript:void(null);">{1}</a>`,
    },
    bookmark_back: {
        name: systemUserName,
        force_avatar: system_avatar,
        ...
        is_system: true,
        is_name: true,
        mes: `이전 채팅으로 돌아가려면 여기를 클릭하세요 : <a class="bookmark_link" file_name="{0}" href="javascript:void(null);">Return</a>`,
    },
};
...
async function Generate(type, automatic_trigger, force_name2) {
    ...
    if (online_status != 'no_connection' && this_chid != undefined && this_chid !== 'invalid-safety-id') {
        ...
        if (this_chid == undefined || this_chid == 'invalid-safety-id') {
            ...
            callPopup('<h3>캐릭터가 선택되지 않았습니다</h3>');
        }
        is_send_press = false;
    }
                ...
                jQuery.ajax({
                    ...
                    success: async function (html) {
                        ...
                        if (true) {
                            ...
                            select_rm_info(`캐릭터가 생성되었습니다!<br><h4>${DOMPurify.sanitize(save_name)}</h4>`, oldSelectedChar);
        ...
        jQuery.ajax({
            ...
            success: async function (data) {
                if (data.file_name !== undefined) {
                    ...
                    await getCharacters();
                    select_rm_info(`캐릭터가 불러와졌습니다<br><h4>${DOMPurify.sanitize(data.file_name)}</h4>`, oldSelectedChar);
