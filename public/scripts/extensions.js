...
function updateStatus(success) {
    ...
    const _text = success ? '확장 기능 API 접속 성공' : '확장 기능 API 접속 끊김';
...
function showExtensionsDetails() {
    let html = '<h3>추가된 확장 기능 :</h3>';
    html += modules.length ? DOMPurify.sanitize(modules.join(', ')) : '<p class="failure">API 와 접속되지 않았습니다!</p>';
    html += '<h3>작동중인 확장 기능 :</h3>';
    ...
    Object.entries(manifests).sort((a, b) => a[1].loading_order - b[1].loading_order).forEach(extension => {
        ...
        if (activeExtensions.has(name)) {
            html += `<p class="success">▲ 확장 기능 켜짐 <a href="javascript:void" data-name="${name}" class="disable_extension">[ 끄기 ]</a></p>`;
            if (Array.isArray(manifest.optional)) {
                const optional = new Set(manifest.optional);
                modules.forEach(x => optional.delete(x));
                if (optional.size > 0) {
                    const optionalString = DOMPurify.sanitize([...optional].join(', '));
                    html += `<p>선택적 확장 기능 : <span class="optional">${optionalString}</span></p>`;
                }
            }
        }
        else if (extension_settings.disabledExtensions.includes(name)) {
            html += `<p class="disabled">▲ 확장 기능 꺼짐 <a href="javascript:void" data-name=${name} class="enable_extension">[ 켜기 ]</a></p>`;
        }
        else {
            const requirements = new Set(manifest.requires);
            modules.forEach(x => requirements.delete(x));
            const requirementsString = DOMPurify.sanitize([...requirements].join(', '));
            html += `<p>확장 기능 없음 : <span class="failure">${requirementsString}</span></p>`
        }
    });
